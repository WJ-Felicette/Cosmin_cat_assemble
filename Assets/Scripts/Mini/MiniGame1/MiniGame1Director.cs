...
public class MiniGame1Director : MonoBehaviour
{
    ...
    public void OnClickDial()
    {
        ...
        DOTween.Sequence()
            .Append(handArr[_side].transform.DOMove(this.DialTarget.position, 0.15f).SetEase(Ease.InCubic))
            .Join(handArr[_side].transform.DORotate(Vector3.forward * 35f * _rotateSide, 0.15f).SetEase(Ease.InCubic))
            .Append(this.Dial.transform.DORotate(Vector3.forward * this.dailID * 72f, 0.2f))
