...
public class WJ_Conn_Mini : MonoBehaviour
{
    // 요청 : 게임 진단 문항요청     
    ...
    protected IEnumerator SendProgress_Diagnosis(string _prgsCd, string _qstCd, string _qstCransr, string _ansrCwYn, long _sid, long _nQstDelayTime)
    {
        ...
        using (UnityWebRequest uwr = UnityWebRequest.Post(url, string.Empty))
        {
            ...
            else
            {
                ...
                {
                    cDiagnotics = JsonUtility.FromJson<DN_Response>(uwr.downloadHandler.text);
                    if (cDiagnotics.data.prgsCd == "E")
                    {
                        Debug.Log("Save ID & Auth");
                        PlayerPrefs.SetString("ID", strMBR_ID);
    ...
    protected IEnumerator GetLearning()
    {
        ...
        using (UnityWebRequest uwr = UnityWebRequest.Post(url, string.Empty))
        {
            ...
            {
                Debug.Log("here + " + uwr.downloadHandler.text);
    ...
    // 학습 결과 전송 (진단평가 이후 진행되는 학습에서 8번 끝나면 실행되는 함수)
    ...
    protected void Make_MBR_ID()
    {
        ...
        strMBR_ID = string.Format("{0}{1:0000}{2:00}{3:00}{4:00}{5:00}{6:00}{7:000}",
                strGameCD, dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, dt.Second, dt.Millisecond);    // 매쓰팜용 리얼 MBR_ID
    }
    ...
    {
        Debug.Log("ID: " + PlayerPrefs.GetString("ID") + " // Auth: "
        + PlayerPrefs.GetString("Auth"));
        PlayerPrefs.DeleteAll();
        Debug.Log("has ID: " + PlayerPrefs.HasKey("ID") + " // has Auth: "
        + PlayerPrefs.HasKey("Auth"));
    }
